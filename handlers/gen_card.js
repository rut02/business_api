...
async function drawCard(data, outputPath) {
    ...
    ctx.fillText(`ตำแหน่ง: ${data.position}`, userDataX, userDataY + userDataSpacing);
    ctx.fillText(`Email: ${data.email}`, userDataX, userDataY + userDataSpacing * 2);
    ctx.fillText(`ที่อยู่: ${data.address}`, userDataX, userDataY + userDataSpacing * 3);
    ctx.fillText(`โทรศัพท์: ${data.phone}`, userDataX, userDataY + userDataSpacing * 4);
    ctx.fillText(`เพศ: ${data.gender}`, userDataX, userDataY + userDataSpacing * 5);
    ctx.fillText(`อายุ: ${data.age}`, userDataX, userDataY + userDataSpacing * 6);
